"""OpenAI-compatible chat witness via llama.cpp server — edge / CPU community nodes."""
...
class LlamaCppVerifier(BaseVerifier):
    """Distributed compute adapter: llama.cpp `llama-server` `/v1/chat/completions`."""

    provider_name = "llama_cpp"

    def __init__(self):
        base = os.getenv("LLAMA_CPP_BASE_URL", "http://127.0.0.1:8080").rstrip("/")
        self.base_url = base
        self.model = os.getenv("LLAMA_CPP_MODEL", "llama")
        self.timeout = float(os.getenv("LLAMA_CPP_TIMEOUT_SECONDS", "120"))

    @property
    def available(self) -> bool:
        return os.getenv("ENABLE_LLAMA_CPP_VERIFIER", "false").lower() == "true"

    async def verify(self, context: dict) -> VerifierResult:
        if not self.available:
            raise RuntimeError("ENABLE_LLAMA_CPP_VERIFIER is not true")
