...
def ensure_human_entity_for_user(db: Session, user: UserAccount) -> tuple[Entity, Wallet]:
    ...
    entity = Entity(
        ...
        metadata_={"provider": user.provider, "provider_user_id": user.provider_user_id},
    )
    db.add(entity)
    db.flush()
    ...
    if wallet.ai_credits <= 0 and wallet.cp_balance <= 0:
        wallet.ai_credits = STARTER_AI_CREDITS
        db.add(
            CreditTransaction(
                wallet_id=wallet.id,
                amount=STARTER_AI_CREDITS,
