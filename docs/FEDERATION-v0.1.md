...
See also: [PROTOCOL.md](./PROTOCOL.md) · [PROTOCOL-SPEC-v0.1.md](../PROTOCOL-SPEC-v0.1.md)
...
| 身份只在本地 UUID 有效 | `portable_id`（如 `github:alice`）跨节点引用 |
...
| POST | `/api/v1/federation/import` | 🔜 501 stub → v0.2 |
...
```json
{
  ...
  "primary_entity_portable_id": "github:alice",
  "contribution_type": "knowledge",
  "evidence": { "url": "...", "_pocp": { "content_hash": "..." } },
  "participants": [
    { "entity_portable_id": "github:alice", "role": "creator", "weight": 0.4 }
  ],
...
| 阶段 | 能力 |
|------|------|
| **V0.1（当前）** | 哈希链账本、证据哈希、导出 API、联邦骨架 |
| **V0.2** | 签名 import、信任权重声誉合并、只读镜像 cron |
| **V1.0** | 多节点生产互认、可选外部锚定（Git tag / IPFS） |
